...
|---|---|---|---|---|---|
| QC — low-quality cell filtering | MAD-based outlier filtering on QC covariates; permissive initial filter | partially_covers | extends (on mito) | Phase 1 p1_mito_threshold (60 dataset-tissues) → quantile-on-data threshold, rule `mito_threshold_compute_from_data`. **Gap:** used quantile-on-data, not MAD per se; only mito covariate swept | durable |
| QC — ambient RNA correction | Correct ambient RNA (SoupX, CellBender, DecontX) | not_audited | — | none | durable → DEFERRED |
